...
func initTemplate() {
	...
	<body><div class="container">
		<div class="sys-header">
			📈 App Usage: <b>{{printf "%.1f" .AppCPU}}%</b> Load | <b>{{printf "%.1f" .AppRAM}}</b> MB RAM | 
			📈 System: <b>{{printf "%.1f" .SysCPU}}%</b> Load | <b>{{printf "%.1f" .SysRAM}}%</b> RAM
		</div>
		
		<div class="card progress-container">
			<div style="display:flex; justify-content:space-between; font-weight:bold;">
				<span>Connection Pool</span>
				<span>{{.TotalActive}} / {{.MaxGlobal}}</span>
			</div>
			<div class="progress-bar">
				<div class="progress-fill" style="width: {{multi .TotalActive .MaxGlobal}}%"></div>
			</div>
		</div>
		...
		<div class="card">
			<div class="header-row">
				<span style="font-weight:bold;">{{.Name}} (Port: {{.Port}})</span>
				<span class="badge">{{.Protocol}}</span>
			</div>
			<table>
				<thead><tr><th width="50%">Backend</th><th class="status-cell">Status</th><th width="20%">Active</th></tr></thead>
				<tbody>
				{{range .Backends}}
				<tr>
					...
					<td class="status-cell">
						{{if .Alive}} 
						<span class="status-box up"><span class="dot">●</span>ONLINE</span>
						{{else}} 
						<span class="status-box down"><span class="dot">○</span>OFFLINE</span> 
						{{end}}
					</td>
		...
		<div class="error-list">
			{{$totalErrors := 0}}
			{{range .Services}}
				{{if .ErrorMsg}}{{$totalErrors = add $totalErrors 1}}<div class="err-msg"><strong>Port {{.Port}}</strong>: {{.ErrorMsg}}</div>{{end}}
				{{range .Backends}}
					{{if not .Alive}}{{$totalErrors = add $totalErrors 1}}<div class="err-msg"><strong>Backend Offline</strong>: {{.Address}}</div>{{end}}
				{{end}}
			{{end}}
			{{if eq $totalErrors 0}}
				<p style="color: #52c41a; font-weight: bold; margin:0;">● All systems operational.</p>
			{{end}}
		</div>
	</div></body></html>`
...
func reloadConfig(isInitial bool) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "[") && strings.HasSuffix(line, "]") {
			section = strings.ToLower(line); continue
		}
		...
		if section == "[global]" {
			...
			if svc, exists := services[port]; exists {
				...
				for _, a := range strings.Split(bAddr, ",") {
					if oldB, ok := oMap[a]; ok { updated = append(updated, oldB) } else {
						updated = append(updated, &Backend{Address: a, Alive: false})
					}
				}
				...
				for _, a := range strings.Split(bAddr, ",") {
					newSvc.Backends = append(newSvc.Backends, &Backend{Address: a, Alive: false})
				}
	...
	if CurrentMonitorPort != oldMPort && !isInitial {
		go startMonitorServer(CurrentMonitorPort)
	}

	for p, s := range services {
		if !newPorts[p] { s.stop(); delete(services, p) }
	}
	if !isInitial { infoLog("Configuration reloaded due to file changes") }
}
	...
	for {
		...
		if time.Since(stat.lastReset) > window {
			...
			if atomic.AddInt32(&stat.count, 1) > atomic.LoadInt32(&connRateLimit) {
				...
				penaltyBox.Store(ip, time.Now().Add(p)); conn.Close(); continue
			}
		}
...
func (s *ProxyService) startHealthCheck() {
	for {
		...
		if interval < 1*time.Second { interval = 3 * time.Second }
		time.Sleep(interval)
		select {
		case <-s.quit: return
		default: s.checkBackendsOnce()
		}
	}
}

func (s *ProxyService) handleConnection(clientConn net.Conn) {
	defer func() {
		if r := recover(); r != nil { errLog("Panic: %v", r) }
		clientConn.Close()
		atomic.AddInt64(&totalActiveConns, -1)
		atomic.AddInt64(&s.CurrentConns, -1)
	}()
	...
	s.mux.RLock()
	proto, backend := s.Protocol, s.getNextAliveBackend()
	s.mux.RUnlock()
	...
	if proto == "http" { timeout = time.Duration(atomic.LoadInt64(&idleTimeoutHTTPNS)) } else {
		timeout = time.Duration(atomic.LoadInt64(&idleTimeoutTCPNS))
	}
	...
	go cp(targetConn, &IdleTimeoutConn{clientConn, timeout})
	go cp(clientConn, &IdleTimeoutConn{targetConn, timeout})
	<-done
}
...
func (s *ProxyService) getNextAliveBackend() *Backend {
	...
	for i := 0; i < len(s.Backends); i++ {
		...
		b := s.Backends[idx]
		b.mux.RLock(); alive := b.Alive; b.mux.RUnlock()
		if alive { return b }
	}
	return nil
}
	...
	for {
		...
		if err == nil {
			...
			if info.ModTime().After(lastModTime) {
				globalMux.Unlock(); reloadConfig(false)
			} else { globalMux.Unlock() }
		}
	}
}
...
func startMonitorServer(port string) {
	...
	go func() {
		var lastAppTime time.Duration
		var lastSysIdle, lastSysKernel, lastSysUser uint64
		for {
			aCPU, sCPU, sRAM := collectOSStats(&lastAppTime, &lastSysIdle, &lastSysKernel, &lastSysUser)
			atomic.StoreUint64(&appCPUUsage, *(*uint64)(unsafe.Pointer(&aCPU)))
			atomic.StoreUint64(&sysCPUUsage, *(*uint64)(unsafe.Pointer(&sCPU)))
			atomic.StoreUint64(&sysRAMUsage, *(*uint64)(unsafe.Pointer(&sRAM)))
			time.Sleep(2 * time.Second)
		}
	}()
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		aCPU := *(*float64)(unsafe.Pointer(&appCPUUsage))
		sCPU := *(*float64)(unsafe.Pointer(&sysCPUUsage))
		sRAM := *(*float64)(unsafe.Pointer(&sysRAMUsage))
		...
		data := struct {
			...
			SysCPU      float64
			SysRAM      float64
		}{services, atomic.LoadInt64(&MaxGlobalConns), atomic.LoadInt64(&totalActiveConns), 
		  aCPU, float64(m.Alloc) / 1024 / 1024, sCPU, sRAM}
